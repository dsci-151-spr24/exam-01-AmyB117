...
*I, Amy Buttles, hereby state that I have not communicated with or gained information in any way from my classmates or anyone other than the Professor or TA during this exam, and that all work is my own.*
...
```
```
***I took code back from lab #03 and switched to fit the code needed.***
...
***I repeated code from question one, and instead switched destination with carrier. The most is YV (Mesa Airlines) and the least is F9 (Frontier Airlines).***
...
**I took inspiration from lab #05 and switched with the arrival delay. The highest arrival mean is 33 (American Airlines), and the lowest is -25 (Delta Airlines).***
...
```{r}
flights %>%
  select(dep_delay, origin, month, day, year) %>%
  arrange(desc(dep_delay))
```

***I took inspiration from Single Data Frames slides but switched what was required and got help with code names from the list above. The highest departure delay was JFK with 1301 and the day was the Jan 9th 2013.***
...
```{r}
flights %>%
  select(air_time, carrier, dest, tailnum) %>%
  arrange(desc(air_time))
```
***Again, reused code and switched to what was required for this question. The longest flight was 695 and was flying to Honolulu airport and the airplane was N77066 on United Airlines, which has 242 seats.***
...
```{r}
flights %>%
  mutate(arrival = arr_delay <= 0 ~ "ontime", arr_delay > 0 ~ "delayed") %>%
ggplot(arrival, aes(x = arrival, y = dep_delay, color = dest)) +
 geom_boxplot() +
 labs(
 x = "Arrival",
 y = "Departure Delay",
...
```
***I was unsure how to figure out how to solve this problem. I took inspiration from the original finding numerical data slides and the lab #02. From the original boxplot, it appears that JFK has the most delays.***
